...
(test basic-problem
  ...
  ;           50 total widgets
  (let ((problem (make-linear-problem (max widgets)
    ...
    (with-tableau-variables (widgets d1 d2 d3)
      (solve-tableau (build-tableau problem))
      (is (<= 45.36 widgets 45.37) (format nil "Computed ~A widgets, instead of 45.36-45.37" (float widgets)))
      (is (<= 2.37 d1 2.38) (format nil "Computed ~A dept 1 runs, instead of 2.37-2.38" (float d1)))
      (is (<= 6.96 d2 6.97) (format nil "Computed ~A dept 2 runs, instead of 6.96-6.97" (float d2)))
      (is (<= 15.37 d3 15.38) (format nil "Computed ~A dept 2 runs, instead of 15.37-15.38" (float d3))))))
